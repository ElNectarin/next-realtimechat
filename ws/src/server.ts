...
wss.on("connection", (ws: WebSocket, req: any) => {
  ...
  const channel: T_WssChannel | undefined = location.query.channel as
    | T_WssChannel
    | undefined;
  ...
  fs.readFile(REGISTERED_USERS_FILE, "utf8", (err, data) => {
    ...
    if (users.includes(searchTerm)) {
      ...
      console.error(
        `User ${handle} is already registered in channel ${E_WssChannel[channel]}`
      );
      return;
    }
  ...
  fs.appendFile(
    logFile,
    `${makeLinePrefix(SYSTEM_USER)} ${handle} has joined the chat\n`,
    "utf8",
    (err) => {
      if (err) {
  ...
  ws.on("close", () => {
    ...
    fs.appendFile(
      logFile,
      `${makeLinePrefix(SYSTEM_USER)} ${handle} has left the chat\n`,
      "utf8",
      (err) => {
        if (err) {
          console.error("Error writing to chat log:", err);
